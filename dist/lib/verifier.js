...
class Verifier {
    ...
    verify() {
        ...
        return that.device
            ...
            if (!ed25519.verify(signature, deviceInfo, that.device.credentials.publicKey)) {
                throw new Error("Signature verification failed");
            }
            let material = Buffer.concat([verifyPublic, Buffer.from(that.device.credentials.pairingId), sessionPublicKey]);
            let keyPair = ed25519.createKeyPair(that.device.credentials.encryptionKey);
            let signed = ed25519.sign(material, keyPair.publicKey, keyPair.secretKey);
